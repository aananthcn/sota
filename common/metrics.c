...
int print_metrics(void)
{
	...
	for(i=0; i< MAX_CAPTURE_TIMES; i++) {
		...
		if((TimeStamps[i].cnt) && ((TimeStamps[i].cnt & 1) == 0)) {
			...
			printf("Time spent in %s is %d mins, %d sec\n",
			       TimeStamps[i].name, time/60, time%60);
		}
	}

	printf("\nMore Metrics:\n------------\n");
	printf("Size of release tar ball - %d\n", DownloadInfo.origsize);
	printf("Size of comp. diff file  - %d\n", DownloadInfo.compdiffsize);
